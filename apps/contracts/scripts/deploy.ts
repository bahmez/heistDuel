...
const argv = minimist(process.argv.slice(2), {
  string: ['network', 'source', 'game-hub', 'admin', 'rust-toolchain', 'upgrade-heist-id', 'env-file'],
  boolean: ['skip-build'],
  default: {
    network:    'testnet',
...
const RUST_TOOLCHAIN: string   = argv['rust-toolchain'] ?? '';
...
if (!UPGRADE_HEIST_ID) {
  ...
  const vkHex = 'abababababababababababababababababababababababababababababababababab';
  const vkRaw = stellar(
    'contract', 'invoke',
    ...NETWORK_ARGS,
    '--source-account', SOURCE,
