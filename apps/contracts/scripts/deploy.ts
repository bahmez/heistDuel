 ...
 *   --vk-file         path to binary VK file from `bb write_vk` for the turn_validity circuit
 *                     (default: apps/circuits/turn_validity/target/vk if it exists)
...
/**
 * Loads the UltraHonk verification key as a hex string.
 *
 * Resolution order:
 *   1. --vk-file CLI option (explicit binary file from `bb write_vk`)
 *   2. apps/circuits/turn_validity/target/vk  (default circuit build output)
 *   3. 32-byte placeholder (for local testing — logs a warning)
 *
 * The VK is generated by running inside apps/circuits/turn_validity/:
 *   nargo compile
 *   bb write_vk -b target/turn_validity.json -o target/vk
 */
function resolveVkHex(): { hex: string; fromFile: boolean } {
  const defaultVkPath = path.join(REPO_ROOT, 'apps', 'circuits', 'turn_validity', 'target', 'vk');

  const candidates: string[] = [];
  if (VK_FILE_ARG) candidates.push(VK_FILE_ARG);
  candidates.push(defaultVkPath);

  for (const candidate of candidates) {
    if (!fs.existsSync(candidate)) continue;

    // bb write_vk outputs a directory with vk/vk and vk/vk_hash inside it.
    // Handle both: if candidate is a directory, look for the inner 'vk' file.
    let filePath = candidate;
    const stat = fs.statSync(candidate);
    if (stat.isDirectory()) {
      const inner = path.join(candidate, 'vk');
      if (!fs.existsSync(inner)) {
        console.warn(`  WARNING: ${candidate} is a directory but contains no 'vk' file.`);
        continue;
      }
      filePath = inner;
    }
    ...
    console.log(`  Loading VK from: ${filePath} (${buf.length} bytes)`);
    return { hex: buf.toString('hex'), fromFile: true };
  }

  console.warn('  WARNING: No VK file found. Using placeholder bytes.');
  console.warn('  To generate the real VK, run inside apps/circuits/turn_validity/:');
  console.warn('    nargo compile');
  console.warn('    bb write_vk -b target/turn_validity.json -o target/vk');
  // 32-byte placeholder — not a valid UltraHonk VK, but allows contract init to succeed.
  return { hex: 'ab'.repeat(32), fromFile: false };
}
